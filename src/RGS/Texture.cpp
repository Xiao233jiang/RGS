...
void Texture::Init()
{
    ...
    stbi_set_flip_vertically_on_load(1);    // 翻转纹理
    stbi_uc* data = nullptr;    // stbi_uc 是无符号字符类型，通常用于表示图像数据
    data = stbi_load(m_Path.c_str(), &width, &height, &channels, 0);    // 加载纹理数据, 0 表示加载所有通道
    ASSERT(data);
